<template>
  ...
  <section class="modulo-contenido" ref="moduloContenidoRef">
    ...
    <div v-if="selectedAction === 'consultar'">
      ...
      <!-- Botones de descarga -->
      <div v-if="casosFiltrados.length > 0" class="sistpec-export-actions">
...
function descargarExcel() {
  const datos = casosFiltrados.value;
  if (datos.length === 0) return;

  const encabezados = ['Número de caso', 'Folio hoja', 'Fecha asignación', 'UPP', 'Especie', 'Total muestras', 'Estatus'];
...
function descargarPDF() {
  ...
  const filasHTML = datos.map(c => {
    ...
    if (c.estatus === 'Concluido') badgeClass = 'activo';
    else if (c.estatus === 'Rechazado') badgeClass = 'inactivo';
